...
class Save extends \Magento\Backend\App\Action {
    ...
    public function execute() {
        ...
        try {
            ...
            $getCustomUrlRewrite = "find-a-builder/index/view/id/".$model->getId();
	     $filterData = [
		UrlRewriteService::TARGET_PATH => $getCustomUrlRewrite
	    ];
	    $rewriteFinder = $this->urlFinder->findOneByData($filterData);
            if($model->getDisplayUrl()){
		    $storeId = 1;
		    if($rewriteFinder){
		    	 $this->urlRewrite->load($rewriteFinder->getUrlRewriteId())
				->setStoreId($storeId)
				->setRequestPath($model->getDisplayUrl())
				->save();
		    }else{
		    	$this->urlRewrite->setStoreId($storeId)
				->setIdPath(rand(1, 100000))
				->setRequestPath($model->getDisplayUrl())
				->setTargetPath($getCustomUrlRewrite)
				->setIsSystem(0)
				->save();
	   	   }
	   }else{
	   ...
	   $model->setStoreIds($this->getRequest()->getParam('stores', []));
           ...
           $this->messageManager->addSuccess(__('You saved the store locator.'));
            ...
            $this->messageManager->addError(__($messages));
