...
class Stores extends \Magento\Framework\View\Element\Template
{
    ...
    public function getStoreCollection()
    {
        ...
        if (!empty($data['location'])) {
                ...
                $tokenData = $this->salesForceLoginNew();
        ...
        if (is_null($this->_storeCollection)) {
            $this->_storeCollection = $this->_getStoreCollection();
         /*  $allowed_ip = '34.215.97.17';  // The IP you want to check

            if ($_SERVER['REMOTE_ADDR'] === $allowed_ip) {
                ini_set('memory_limit', '2G');  // Increase the memory limit to 1GB

                echo 'test';
                die(is_null($this->_storeCollection));
    ...
    private function salesForceLoginNew()
    {
        $response = array();
        $url = 'https://login.salesforce.com/services/oauth2/token';
        $params = [
            ...
            'client_secret' => '10FA4E71B471F649958E0D2A730250CB529E128517AA1DF9BC44E0C457C824AC',
            'format' => 'json',
            'username' => 'sfadmin@wausauhomes.com',
            'password' => 'Cafe@1235'
        ];
